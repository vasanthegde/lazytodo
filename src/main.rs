...
fn main() {
    ...
    let mut todos: HashMap<String, Vec<Todo>> =
    ...
    match todo.command.unwrap() {
        ...
        Commands::List { range } => {
            let ls: Vec<_> = todos
                .iter()
                .filter(|x| isValidBucket(x.0.clone(), range.unwrap_or(Bucket::ALL)))
                .flat_map(|x| x.1)
                .collect();
            println!("{:?}", ls)
        }
...
fn isValidBucket(key: String, bucket: Bucket) -> bool {
    ...
    match bucket {
        Bucket::TODAY => return today == key_date,
        Bucket::WEEK => return key_date >= today.checked_sub_signed(Duration::days(7)).unwrap(),
        Bucket::MONTH => return key_date >= today.checked_sub_signed(Duration::days(30)).unwrap(),
        _ => return false,
    }
}
